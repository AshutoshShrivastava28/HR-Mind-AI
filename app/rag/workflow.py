...
def retrieve_kb(state: AgentState):
    ...
    return {
        "kb_docs": docs,
        "trace": add_trace(state, f"Private KB retrieval → {len(docs)} chunks"),
    }
...
You are grading evidence for an enterprise HR assistant.
...
If the knowledge base contains information that directly answers the question,
return "good".

For example, if the question asks about remote work and the knowledge base
contains a Remote Work Policy with eligibility, frequency, approval, working
hours, tools, or security requirements, that is sufficient evidence.

Return "weak" only when the knowledge base truly does not contain enough
...
def grade_web(state: AgentState):
    ...
    grade = grader.invoke(f"""
You are evaluating web search evidence for an HR employee-support assistant.
...
Evaluate whether the web evidence contains enough directly relevant information to answer the user's question.

Return "good" when:
- at least one result is directly relevant to the question, and
- the evidence contains useful factual information that can support an answer.

Return "weak" only when the results are irrelevant, contradictory, or provide insufficient information.

Do not require the evidence to contain the exact wording of the question.
...
""").content
    ...
    for d in state["kb_docs"]:
        ...
        if src not in seen:
            seen.add(src)
            citations.append(
                {"title": src.split("/")[-1], "url": "", "type": "private_kb"}
            )
    return {
        "answer": answer,
...
def direct_answer(state: AgentState):
    answer = (
        llm().invoke(f"Respond briefly and naturally to: {state['question']}").content
    )
    return {
        "answer": answer,
        "source_used": "direct",
